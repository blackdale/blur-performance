...
//            "}";
     ...
     */
    public Square(Context context, int width, int height) {
        mContext = context;
        prepareBuffers();
        preparePrograms();
        prepareTexture();
    ...
    private void preparePrograms() {
        ...
        int vertexShader = MyGLRenderer.loadShader(
                ...
                vertexShaderCode);
        int fragmentShaderHorizontal = MyGLRenderer.loadShader(
                GLES31.GL_FRAGMENT_SHADER,
                horFragmentShaderCode);

        int verticalShaderHorizontal = MyGLRenderer.loadShader(
                GLES31.GL_FRAGMENT_SHADER,
...
//            GLES31.glTexParameteri(GLES31.GL_TEXTURE_2D, GLES31.GL_TEXTURE_MAX_LEVEL, 6);
    ...
    private void draw(ProgramData aProgramData, float width, float height) {
        ...
        GLES31.glUniform1f(aProgramData.mMipMapHandle, mMipMap);
        GLES31.glUniform1f(aProgramData.mRadiusHandle, mRadius);
    ...
    public void draw() {
        ...
        draw(mVerticalProgramData, mWidth, mHeight);
        // Make vertical blur pass (render on screen)
        draw(mHorizontalProgramData, mWidth, mHeight);
    }
}